    ...
    def _apply_dbscan(self, df, eps=0.01, min_samples=5, bbox=None, max_time_diff_days=5):
        ...
        if 'acq_date' in df.columns:
            try:
                ...
                time_scaling = 0.5 / max_time_diff_days  # Inverse of max days difference
...
def export_all_clusters_timeline(df, category, playback_dates, basemap_tiles, basemap):
    ...
    for i, date in enumerate(sorted(playback_dates)):
        ...
        date_data = valid_data[valid_data['acq_date'] == date].copy()
    ...
    if frames:
        # Create download buffer
        st.info("Timeline export ready for all clusters")
        st.download_button(
            label="Download as GIF",
            data=create_gif_from_frames(frames),
    ...
    with main_cols[0]:
        ...
        if generate_button:
            with st.spinner("Analyzing fire data..."):
                ...
                # INSERT THE MULTI-DAY FILTERING CODE HERE
                if show_multiday_only and results is not None and not results.empty and 'acq_date' in results.columns:
